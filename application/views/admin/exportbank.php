<script type="text/javascript" src="<?php echo base_url("assets/js/app/exportbank.js")?>"></script>
   ...
   <div class="row">
      <div class="container">
         <div class="col s12 m6 l6">
            <div class="pagebannertext white-text" >Export Bank Statement</div>
         </div>
         <div class="col s12 m6 l6 right-align">
            <div class="dumheight hide-on-small-only"> </div>
            <div class=""> <a href="<?php echo base_url('admin/dashboard')?>" class="breadcrumb">Home</a> <span class="breadcrumb">Bank Statement</span> </div>
         </div>
      </div>
   </div>
   <div class="parallax"><img src="<?php echo base_url("assets/images/breadcrumbbanner.jpg")?>"></div>
</div> 
<div  class="row" ng-app="appExportBankSlip" style="padding-top:30px;">
    <div  class="container" ng-controller="ctrlExportBankSlip">
        <div class ="col s12 m12 l12" style="margin-bottom: 25px;">
            ...
            <div class="input-field col s3" style="padding-left: 10%;" ng-show="invoicecount > 1">
                <a ng-click="ExportAllBankXMLFormat(invoice.monthandyear,baseurl)" class="btn cyan waves-effect waves-light" type="submit">Export All </a>
            </div>
        </div> 
        <div style="margin-bottom: 25px;" ng-show="invoicecount">
            <table ng-table="tableParams" class="responsive-table highlight" fixed-table-headers="scrollable-area" show-filter="true" class="bordered">
                <tr ng-repeat="invoice in $data">
                    ...
                    <td width="25%" data-title="'Project No'" filter="{projectno: 'number'}" sortable="'projectno'">{{invoice.projectno}}</td> 
                    <td width="25%" data-title="'Project Name'" filter="{projectname: 'text'}" sortable="'projectname'">{{invoice.name}}</td>
                    <td width="10%" data-title="'Invoice No'" filter="{invoiceno: 'number'}" sortable="'invoiceno'"> {{invoice.invoiceno}}</td>
                    <td width="10%" data-title="'Month and Year'" sortable="'monthandyear'"> {{invoice.monthandyear}}</td>
                    <td width="15%" data-title="'Export'" style="border-right:10px;">
                        &nbsp; &nbsp;&nbsp; &nbsp;
                        <a class="btn cyan waves-effect waves-light green" ng-show="invoice.active==1" title="Export Invoice" ng-click="ExportBankStatement(invoice.invoiceid,invoice.invoiceno,invoice.monthandyear,invoice.clientid,invoice.projectid,baseurl)">Export Statement</a>
                    </td>
                </tr>
            </table>
        </div>
        <div style="margin-bottom: 25px;" ng-show="invoicecount == 0">
            <table ng-table="tableParams" class="responsive-table highlight" fixed-table-headers="scrollable-area" show-filter="true" class="bordered">
                ...
                <tbody>
                    <tr class="ishold"><td colspan="4" style="text-align:center;">No data found. Select month to view invoice</td></tr>
                </tbody>
            </table>
        </div>
    </div>
