<script type="text/javascript" src="
   <?php echo base_url("assets/js/app/bulkprint.js")?>"></script>
    ...
    <div class="row">
        <div class="container">
            <div class="col s12 m6 l6">
                <div class="pagebannertext white-text">Bulk Print</div>
            </div>
            <div class="col s12 m6 l6 right-align">
                ...
                <div class="">
                  ...
                  <?php echo base_url('admin/dashboard')?>" class="breadcrumb">Home
                    ...
                    <span class="breadcrumb">Bulk Print</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div>
            ...
            <div class="input-field col m2 s6" ng-init="objattendance.type='1'">
                <select id="type" name="region" class="validate" ng-model="objattendance.type"
                    ng-required="true">
                    <option value="1"> Salary Bill </option>
                    <option value="2"> Salary Claim </option>
                    <option value="3"> Arrear Salary Bill</option>
                    <option value="4"> Arrear Claim </option>
                </select>
                <label for="Type">Type</label>
            </div>
            <div class="input-field col m2 s6">
                <button class="btn waves-effect waves-light  cyan darken-2" type="submit"
                    ng-click="bulkprint(objattendance)">Search</button>
            </div>
        </div>
        <div id="" class="section" ng-if="objattendance.type == '1' && showdata=='1'">
            <div id="sup-details-pending" class="col s12">
                <table ng-table="firstTableParamsub" class="responsive-table highlight striped" show-filter="true"
                    ...
                    <tr ng-repeat="applied in $data">
                        <td style="text-align: left" width="7%">
                            <input type="checkbox" class="filled-in" id="ishold_{{applied.payslipno}}"
                                name="ishold_{{applied.payslipno}}" ng-checked="applied.ishold"
                        ...
                        <td width="8%" data-title="'Payslip Number'" filter="{payslipno: 'text'}">{{applied.payslipno}}
                        </td>
                        <td width="20%" data-title="'Wage Category'" filter="{projectname: 'text'}"
                            sortable="'projectname'">{{applied.wagecategory}}</td>
                        <td width="12%" data-title="'Wage Year'" filter="{monthandyear: 'text'}"
                            sortable="'monthandyear'">{{applied.wageyear}}</td>
                        <td width="12%" data-title="'Status'" filter="{status: 'text'}" sortable="'monthandyear'">
                            {{applied.currentstatus}}</td>
                ...
                <div ng-show="selectedproj.length > 0">
                    <div class="input-field col m3 s6">
                        <select id="cat" name="region" class="validate"
                            ng-class="{'submitted': submitted && attendanceform.projectno.$invalid}" ng-model="category"
                            data-ng-options=" r as r.category_description for r in wgcategory"
                    ...
                    <div class="input-field col m3 s6">
                        <button class="btn waves-effect waves-light  cyan darken-2" type="submit"
                            ng-click="proceedarrear(category)" ng-disabled="isDisabled">Approve</button>
                    </div>
                </div>
            </div>
            ...
            <div id="sup-details-pending" class="col s12">
                <table ng-table="firstTableParamsub" class="responsive-table highlight striped" show-filter="true"
                    ...
                    <tr ng-repeat="applied in $data">
                        ...
                        <td style="text-align: left" width="7%">
                            <input type="checkbox" class="filled-in" id="ishold_{{applied.invoiceno}}"
                                name="ishold_{{applied.invoiceno}}" ng-checked="applied.ishold"
                                ng-model="applied.ischecked" ng-change="selectEntity(applied)" />
                        ...
                        <td width="5%" data-title="'S.No'">{{$index + 1}}</td>
                        <td width="8%" data-title="'Invoice No'" filter="{invoiceno: 'text'}"> <a
                                href="<?php echo base_url('admin/printclaiminvoice?invoiceno={{applied.invoiceno}}')?>"
                                target="_blank">{{applied.invoiceno}}</a></td>
                        <td width="8%" data-title="'Wage Category'" filter="{wagecategory: 'text'}">
                            {{applied.wagecategory}}</td>

                        <td width="8%" data-title="'Wage Year'" filter="{wageyear: 'text'}">{{applied.wageyear}}</td>

                        <td width="20%" data-title="'Service Charge'" filter="{servicecharge: 'text'}"
                            sortable="'servicecharge'">{{applied.servicecharge}}</td>
                        <td width="12%" data-title="'Tax'" filter="{tax: 'text'}" sortable="'tax'">{{applied.tax}}</td>
                        <td width="12%" data-title="'Status'" filter="{status: 'text'}" sortable="'monthandyear'">
                            {{applied.Status}}</td>
                ...
                <div ng-show="selectedproj.length > 0">
                    <div class="input-field col m3 s6">
                        <select id="wgcat" name="category" ng-model="category"
                            data-ng-options=" r as r.category_description for r in wgcategory" ng-required="true">
                        </select>
