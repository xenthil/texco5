...
class Member extends CI_Controller {
	...
	public function dashboard()
	{
		if ($this->session_check() != false) {
			...
			$this->load->view('member/dashboard',$memdata);
	...
	public function skilldevelopment()
	{
		if ($this->session_check()!=false){
			$auth['membername'] = $this->session->userdata('membername');
			$this->load->view('shared/memberheader', $auth);
			$this->load->view('member/skilldevelopment');
			$this->load->view('shared/memberfooter');
		}
		else
			{
	...
	public function vacancydocupload()
    {
        $json                    = array();
        $config['upload_path']   = 'assets/vacancy/members';
	...
	public function vacancyapply()
	{
		if ($this->session_check()!=false){
			...
			$memdata['memberid'] = $this->session->userdata('memberid');
			$this->load->view('shared/memberheader', $auth);
			$this->load->view('member/vacancyapply',$memdata);
			$this->load->view('shared/memberfooter');
		}
		else
			{
   ...
   $memurldata = $this->Get_operation($memurl);
	...
	function check_login_vacancy()
	{
		$requestval = $_POST['data'];
		//echo $requestval;
		$url =$this->config->item('api_url'). '/memberinfovacancy?texserno='.$requestval;		 
		$response_opt = $this->Get_operation($url);
		if ($response_opt['httpcode'] != 200) {
            echo $response_opt['output'];
        } else {
