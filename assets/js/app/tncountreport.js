var app = angular.module('appTncountreport', ['ngSanitize', 'ngCsv', 'ngTable', 'ui.bootstrap']);
app.constant("moment", moment);
app.controller('ctrlTncountreport', function ($scope, $http, $filter, NgTableParams) {
    ...
    $scope.submittncount = function (objproject) {
        $http({
            method: 'POST',
            data: $.param({
    ...
    $scope.exporttncount = function (objproject) {
        $http({
            method: 'POST',
            data: $.param({
                startdate: objproject.fromdate,
            ...
            if (response.status = 200) {
                var data = response.data;
                JSONToCSVConvertor(data, 'TN_Count_report', true)
            } else {
                Materialize.toast('error', 3000, 'red');
            }
        }, function errorCallback(response) {
