...
app.controller('ctrlPrint', function($scope, $http) {
    ...
    if(clientid)
    {
        $http({
            ...
            url: api_url + "/job/get/printemployees?clientid="+clientid+"&types="+types,
    ...
    $scope.gettotal = function(data){
        ...
        $scope.totalgross = Number($scope.edamount) + Number($scope.basic) + Number($scope.ma) + Number($scope.unifdt) + Number($scope.leapay) + Number($scope.bonus) + Number($scope.washallow) + Number($scope.other1); 
        $scope.totaldeduc = Number($scope.epf) + Number($scope.other2) ; 
        $scope.netpay = Number($scope.totalgross) - Number($scope.totaldeduc)- Number($scope.epf) ; 
        $scope.ntpay=$scope.basic +  $scope.ma + $scope.unifdt + $scope.leapay + $scope.bonus + $scope.washallow +  $scope.other1 - $scope.epf - $scope.other2;
        $scope.inwords = $scope.numberInWords(Math.round($scope.ntpay));
    }
