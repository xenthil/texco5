...
//     "domain": "link",
//     "uploader_id": null,
//     "created_at": "2024-05-20T03:26:15Z",
...
$workableData = json_decode(file_get_contents('php://input'), true);





  //$workableData=json_decode($data,true);
...
if($workableData['data']['stage'] == "Qualified")
{

  file_put_contents('logs.txt', json_encode($workableData).PHP_EOL , FILE_APPEND | LOCK_EX);
...
$finaldata['recruitingCampaignId']='907F2CBDB5EE7E445664959C8EC3E4F787CB607174542B711D4DAC7A791ED4ED';
...
//echo $yearsexp=$workableData['data']['answers'][$i]['answer']['choices'][0]['body'];
  ...
  $DegreeLevel['name']='Highest level of education obtained';
  ...
  if($resourceJobTitle=='Math')
{
  ...
  $Yrs_of_Exe['name']='Math - Years of Experience';
  $Yrs_of_Exe['values']=$yearsexp;
}
else{
  
  $Yrs_of_Exe['name']='Reading - Years of Experience';
  $Yrs_of_Exe['values']=$yearsexp;
}
...
if($resourceJobTitle=='Math')
{
  $Certs['name']='Math - Certifications';
  $Certs['values']=$certifications;
}
else{
  $Certs['name']='Reading - Certifications';
  $Certs['values']=$certifications;
}
  ...
  $languagess['name']='Language Fluency';
  ...
  if($resourceJobTitle=='Math')
{
  $dateapp['name']='Math - Application Date';
  $dateapp['values']=$AppDates;

  $pstype['name']='Position Type';
  $pstype['values']= array('Math');


}
else{
  $dateapp['name']='Reading - Application Date';
  $dateapp['values']=$AppDates;
  $pstype['name']='Position Type';
  $pstype['values']=array('Math');
}
...
array_push($profileFieldUpdates,$pstype);

  $finaldata['profileFieldUpdates']=$profileFieldUpdates;


  $finaldata['resourceAddress1']= $finaldata['resourceCity'].','. $finaldata['resourceState'].','.$finaldata['resourceZip']; 

  file_put_contents('13.txt',json_encode($finaldata));
...
$headers = array(
  ...
  'Authorization: Bearer gTQeMNqTs1dltI4x'
);
$ch      = curl_init(); 
curl_setopt($ch, CURLOPT_URL,'https://booknook.wm.solutions/createAccount/201/3117');
...
file_put_contents('response.txt',json_encode($result));
}
