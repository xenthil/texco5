...
/* SQL Error (1822): Failed to add the foreign key constaint. Missing index for constraint 'FK__attendance_other_expense_attendance' in the referenced table 'jobmaster' */
...
ALTER TABLE bank_slip_details
ADD CONSTRAINT FK__bank_slip_details_salaryslip
FOREIGN KEY (salaryslipno) REFERENCES salaryslip(payslipno);
/* SQL Error (1822): Failed to add the foreign key constaint. Missing index for constraint 'FK__bank_slip_details_salaryslip' in the referenced table 'salaryslip' */
...
/* SQL Error (1822): Failed to add the foreign key constaint. Missing index for constraint 'FK__project_subcategory' in the referenced table 'subcategory' */
...
FOREIGN KEY (subcategoryid) REFERENCES subcategory(subcategoryid);
/* SQL Error (1822): Failed to add the foreign key constaint. Missing index for constraint 'FK__project_ams_subcategory' in the referenced table 'subcategory' */
...
FOREIGN KEY (wageyearid) REFERENCES lookupvalue(lkvalid);
/* Affected rows: 0  Found rows: 0  Warnings: 0  Duration for 138 queries: 00:01:20.3 */